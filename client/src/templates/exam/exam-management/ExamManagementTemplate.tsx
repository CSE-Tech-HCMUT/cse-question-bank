...
import { createExamThunk, getAllExamsThunk, previewPDFFileThunk } from "@/stores/exam/thunk";
...
export const ExamManagementTemplate = () => {
    ...
    const handleClickCreateExam = () => {
        dispatch(createExamThunk({
            ...
            if (actionResult.meta.requestStatus === 'fulfilled') {
                ...
                if (idExam) {
                navigate(PATH.EXAM_CREATION.replace(':subjectName', subjectAuthen?.name!) + '/' + idExam);
                } 
            }
            }
        )
    ...
    const columns: ColumnsType<Exam> = [
        ...
        },
        {
            ...
            key: "name",
            className: "!text-center",
            render: (_text: string) => <span className="text-primary">{"Cuối kì"}</span>
        },
        {
            title: "Học kì",
            dataIndex: "semester",
        ...
        },
        {
            ...
            key: "date",
            className: "!text-center",
            render: (_text: string) => <span className="text-primary">{new Date("2025-05-17").toLocaleDateString()}</span>
        },
        {
            title: "Thao tác",
            key: "actions",
    ...
    return (
        <main className="bg-gray-100 rounded-md">
            ...
            <div className="bg-white p-4 rounded-md shadow-md">
                ...
                title={TitleTable}
                columns={columns}
                dataSource={data?.filter((exam) => exam.subject?.id === subjectAuthen?.id)}
