...
const SingleQuestionTemplate: React.FC<QuestionCreationProp> = ({
  idQuestion,
  subjectAuthen,
  isParent,
  parentId,
  ...
  const [contentAnswerAutoQuestion, setContentAnswerAutoQuestion] =
    useState<string>("");
  const [numberOfDistractionAnswers, setNumberOfDistractionAnswer] =
    useState<number>(0);
  ...
  const handleTagAssignmentChange = (tagId: number, optionId: number) => {
    setTagAssignments((prev) => {
      const existingAssignmentIndex = prev.findIndex(
        (assignment) => assignment.tag?.id === tagId
      );
      if (existingAssignmentIndex >= 0) {
        const updatedAssignment = [...prev];
        updatedAssignment[existingAssignmentIndex] = {
          ...updatedAssignment[existingAssignmentIndex],
          option: { id: optionId },
        };
    ...
    dispatch(editQuestionThunk(payload)).then((actionResult) => {
      if (actionResult.meta.requestStatus === "fulfilled") {
        isParent
          ? onCloseModal!()
          : navigate(
              PATH.QUESTION_MANAGEMENT.replace(
                ":subjectName",
                subjectAuthen?.name!
              )
            );
    ...
    >
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: "24px" }}>
        <Col xs={24}>
          <Card
            style={{ height: "100%" }}
            title={"2. Nhập thông tin cho các nhãn của câu hỏi"}
          >
            <Row gutter={[16, 16]} style={{ marginBottom: "12px" }}>
              {tagData?.filter(
              ...
              {mode === "auto" ? (
                <>
                  ...
                  <Col xs={24}>
                    ...
                    <TextEditor
                      onChange={handleEditorChangeContentQuestion}
                  ...
                  <Col xs={24}>
                    ...
                    <TextEditor
                      onChange={handleEditorChangeContentAnswerAutoQuestion}
                    />
                  </Col>
                  ...
                  <Col xs={24}>
                    ...
                    <Input
                      type="number"
                      placeholder={t("placeholderDistractAnswer")}
                      onChange={(event) => {
                ...
                <>
                    ...
                    <Col xs={24} key={index}>
                      ...
                      {editorMode === "text" ? (
                        <TextEditor
                          ...
                          onChange={(newContent) =>
                            handleManualAnswerChange(index, newContent)
                          }
                      ...
                      <Checkbox
                        ...
                        onChange={(e) =>
                          handleManualAnswerCheckChange(index, e.target.checked)
                        }
                      >
                        {t("correct")}
                      </Checkbox>
                  ...
                  <Button type="dashed" onClick={handleAddManualAnswer}>
                    {t("addAnswer")}
                  </Button>
                </>
              ) : (
